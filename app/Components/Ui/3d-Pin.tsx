...
export const PinPerspective = ({
  ...
  // State to ensure rendering happens only on the client
  const [isMounted, setIsMounted] = useState(false);

  useEffect(() => {
    // Set isMounted to true when the component is mounted on the client
    setIsMounted(true);
  }, []);
      ...
      >
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <a
            href={href}
            target={"_blank"}
            className="relative flex space-x-2 items-center block z-10 rounded-full bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10"
          >
